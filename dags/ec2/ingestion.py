"""
Monthly NYC taxi ingestion pipeline.
...
def run_fetch_weather(**context):
    ...
    s3_key = sync_weather_month_to_s3(month, skip_if_exists=True)
    context["ti"].xcom_push(key="weather_s3_key", value=s3_key)
...
def run_prune(**context):
    ...
    deleted = prune_s3_months(latest_month=month, keep_months=12)
    context["ti"].xcom_push(key="deleted_keys", value=deleted)
...
def run_pytest_validation(**context):
    ...
    result = subprocess.run(
        [
            "python", "-m", "pytest",
            f"{AIRFLOW_ROOT}/tests/test_data_validation_ec2.py",
            "-v", "--tb=short", "-rxs",
        ],
        env={**os.environ, "TEST_MONTH": month, "PYTHONPATH": pythonpath},
        capture_output=True,
        text=True,
    )
...
with DAG(
    dag_id="taxi_data_ingestion_ec2",
    description="Monthly taxi ingestion pipeline (EC2, flat layout)",
    schedule="@monthly",
    ...
    trigger_training = TriggerDagRunOperator(
        ...
        trigger_dag_id="taxi_model_training_ec2_self_stop",
        wait_for_completion=False,
    )

    get_month >> prune_old_data >> [download_data, fetch_weather] >> validate_data >> merge_everything >> features >> trigger_training
